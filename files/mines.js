...
function odhalitBunku(bunka, symbol) {
    ...
    if (symbol === '💣') { // Kontrola, zda hrac klikl na bombu
        zobrazitZpravaBox('Prohrál jsi! Klikni na Reset pro novou hru.'); // Zobrazení zprávy
        disableAllCells(); // Deaktivace dalsi interakce po prohre
    }
}

// Funkce pro zobrazení boxu se zprávou
function zobrazitZpravaBox(zprava) {
    const zpravaBox = document.createElement('div');
    zpravaBox.classList.add('zprava-box');

    const zpravaContent = document.createElement('div');
    zpravaContent.classList.add('zprava-content');
    ...
    closeButton.addEventListener('click', () => {
        zpravaBox.remove(); // Odstranění boxu po kliknutí
    });

    zpravaContent.appendChild(text);
    zpravaContent.appendChild(closeButton);
    zpravaBox.appendChild(zpravaContent);

    document.body.appendChild(zpravaBox); // Přidání boxu do stránky
}
